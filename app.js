...
app.post("/post_form", (req, res) => {
  pool.getConnection(function (err, connection2) {
    if (err) throw err;
    let query2 =
      "INSERT INTO customers (CustUserID, CustPasswd, CustName, CustAddress, CustCity, CustProv, CustPostal, CustPhone, CustEmail, CustMsg ) VALUES (?,?,?,?,?,?,?,?,?,?) ";
    let hashpw = crypto
      .pbkdf2Sync(req.body.password, hashsalt, 1000, 64, `sha512`)
      .toString("hex");
    let data1 = [
      req.body.userID,
      hashpw,
